...
Deno.serve(async (req) => {
  ...
  try {
    ...
    $('tr').each((_, row) => {
      ...
      if (!drawNumberEl.length) return;

      const drawNumber = drawNumberEl.text().trim();
      const drawDateText = drawNumberEl.next().text().trim();
    ...
    for (const result of results) {
      ...
      if (result.lakshapathiNumbers) {
        ...
        if (lakshapathiError) throw lakshapathiError;
      }
      ...
      if (result.millionaireNumbers) {
        ...
        if (millionaireError) throw millionaireError;
      }
      ...
      if (result.fiveHundredKNumbers) {
        ...
        if (fiveHundredKError) throw fiveHundredKError;
      }
    }
